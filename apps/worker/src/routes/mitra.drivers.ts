...
mitraDriverRoutes.post(
  '/',
  zValidator('json', createDriverPayloadSchema),
  async (c) => {
    ...
    try {
      ...
      const existingDriver = await db
        ...
        .where(and(
          ...
          eq(drivers.identifier, payload.identifier)
        ))
        .limit(1);
      ...
      const [createdDriver] = await db.insert(drivers).values({
        id: newDriverId,
        mitraId: mitraId,
 ...
 * Update a specific driver (must belong to authenticated Mitra)
 */
mitraDriverRoutes.put(
  '/:driverId',
  zValidator('json', updateDriverPayloadSchema),
  async (c) => {
    const { driverId } = c.req.param();
    const payload = c.req.valid('json');
    const mitraId = c.get('currentMitraId')!;
    ...
    try {
      // If identifier is being changed, check for conflicts
      if (payload.identifier) {
        const conflictingDriver = await db
          .select({ id: drivers.id })
          .from(drivers)
          .where(and(
            eq(drivers.mitraId, mitraId),
        ...
        if (conflictingDriver.length > 0 && conflictingDriver[0].id !== driverId) {
          return c.json({
            success: false,
            error: {
              code: 'CONFLICT',
              message: `Another driver with identifier '${payload.identifier}' already exists.`,
            },
          }, 409);
        }
      }
    ...
    try {
      ...
      const existingAssignment = await db
        ...
        .where(and(
          ...
          eq(driverServices.serviceId, serviceId)
        ))
        .limit(1);
